...
bool obj_string_is_truthy(const obj_string_t* self) {
  assert(0 && "todo: implement");
}
...
obj_t* obj_string_apply(const obj_string_t* self, obj_t* args, obj_env_t* env) {
    throw(obj_string_new_cstr("string is not callable"), (obj_t*) self);
}
...
char obj_string_at(const obj_string_t* self, size_t index) {
    if (self->top <= index) {
        throw(obj_string_new_cstr("index out of bounds"), (obj_t*) self, (obj_t*) obj_u32_new(index));
    }
